...
Class AzNamingStandards{
    ...
    static [string]GetManagementGroupName([PfAzureContext]$PfAzureContext){
        return "MG-$($PfAzureContext.CompanyInfo.Abbreviation)-Root"
    }
    static [string]GetSubscriptionName([PfAzureContext]$PfAzureContext){
        return "SUB-$($PfAzureContext.CompanyInfo.Abbreviation)-$($PfAzureContext.Group.Abbreviation)-$($PfAzureContext.Environment.Letter)"
    }
